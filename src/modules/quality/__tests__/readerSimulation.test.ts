...
describe("runReaderSimulation", () => {
  ...
  it("calls reader_simulation task type (OpenAI routing)", async () => {
    mockedCall.mockResolvedValueOnce(llm(validLLMResponse()));
    await runReaderSimulation(baseInput());
    expect(mockedCall).toHaveBeenCalledWith("reader_simulation", expect.stringContaining("pacing-intolerant binge reader"), expect.any(Object));
  });
  ...
  it("calculates composite score from sub-scores", async () => {
    ...
    // 10*0.35 + 10*0.25 + 10*0.25 + 9*0.15 = 9.85
  ...
  it("retries on parse failure up to MAX_RETRIES", async () => {
    mockedCall
      .mockResolvedValueOnce(llm("not json"))
      .mockResolvedValueOnce(llm("still bad"))
      .mockResolvedValueOnce(llm(validLLMResponse()));
    const result = await runReaderSimulation(baseInput());
...
describe("runSampleReadGate", () => {
  ...
  it("returns acquisition decision from LLM response", async () => {
    mockedCall.mockResolvedValueOnce(llm(JSON.stringify({
  ...
  it("uses reader_simulation task type", async () => {
    mockedCall.mockResolvedValueOnce(llm(JSON.stringify({
      acquisition_decision: "MARGINAL", reasons: [], strongest_element: "", weakest_element: "", recommendation: "",
    })));
  ...
  it("throws after all retries fail", async () => {
    mockedCall.mockResolvedValue(llm("bad"));
